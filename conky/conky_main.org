...
# README
# run "ip route" at command line to identify wired and wireless interfaces
# perform a search and replace for:
...
${voffset 5}${color1}OS: ${color7}${execi 10000 lsb_release -d | cut -f 2| tr "[:upper:]" "[:upper:]"}
${voffset 5}${color1}Kernel: ${color7} $sysname $kernel
...
${color1}IP address: ${color7}${addr wlp2s0}
#${execi 3600 ip route | grep default | sed -e "s/^.*dev.//" -e "s/.proto.*//"}
#
# Warning! do not show external ip address in screenshots you post, disable or obscure in your screenshot
# ${color1}External IP: ${color7}${execi 3600 wget -q -O - checkip.dyndns.org | sed -e 's/[^[:digit:]\|.]//g'}
#
 ${color1}Wireless signal: ${color7}${wireless_link_qual_perc wlp2s0}%
 ${color1}Connections: ${color7}${tcp_portmon 1 65535 count}
 ${color1}Download speed: ${color7}${downspeed wlp2s0}
#
# netw graphs
${voffset 5}${color6}${downspeedgraph wlp2s0 35,135 34495E ffffff 17000 -l} $alignr${upspeedgraph enp3s0 35,135 34495E ffffff 3000 -l}
#

${voffset -5}${color1}Received: ${alignr 5}Sent:
${color1}Session:${color7}${goto 75}${totaldown wlp2s0} ${alignr 5}${totalup wlp2s0}

# Stat - don't really need
# https://www.howtoforge.com/tutorial/vnstat-network-monitoring-ubuntu/
#${color1}Today:${color7}${goto 75}${execi 60 vnstat -i wlp2s0 | grep "today" | awk '{print $2 $3}'}${alignr 5}${execi 60 vnstat -i wlp2s0 | grep "today" | awk '{print $5 $6}'}
#${color1}Week:${color7}${goto 75}${execi 60 vnstat -i wlp2s0 -w | grep "current week" | awk '{print $3 $4}'}${alignr 5}${execi 60 vnstat -i wlp2s0 -w | grep "current week" | awk '{print $6 $7}'}
#${color1}Month:${color7}${goto 75}${execi 60 vnstat -i wlp2s0 -m | grep "`date +"%b '%y"`" | awk '{print $3 $4}'}${alignr 5}${execi 60 vnstat -i wlp2s0 -m | grep "`date +"%b '%y"`" | awk '{print $6 $7}'}
#
# Processes
